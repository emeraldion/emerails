...
class BaseController
{
    ...
    protected function initialize()
    {
        ...
        $this->name = camel_case_to_joined_lower(
    ...
    protected function cached_page_filename()
    {
        ...
        $cachefile =
            dirname(__FILE__) .
            "/../caches/{$this->name}/{$this->action}{$id}-{$lang}.cached";
        return $cachefile;
    }
    ...
    protected function expire_cached_page($params)
    {
        ...
        foreach ($langs as $lang) {
            $cachefile =
                dirname(__FILE__) .
                "/../caches/{$controller}/{$action}{$id}-{$lang}.cached";
            @unlink($cachefile);
        }
    }
    ...
    function url_to_myself($relative = true)
    {
        ...
        return $relative
            ? $url
            : sprintf('http://%s%s', $_SERVER['HTTP_HOST'], $url);
    }
    ...
    public function render($params)
    {
        ...
        if ($params === null) {
            ...
            // Get part file
            $partfile =
                dirname(__FILE__) . "/../views/{$this->name}/_{$partial}.php";
            ...
            $action =
                isset($params['action']) && !empty($params['action'])
                    ? basename($params['action'])
                    : $this->action;

            // Get part file
            $partfile =
                dirname(__FILE__) . "/../views/{$this->name}/{$action}.php";
            ...
            if (
                (isset($params['layout']) && $params['layout'] == false) ||
    ...
    {
        $layout =
            isset($params['layout']) && !empty($params['layout'])
                ? basename($params['layout'])
                : $this->name;
        ...
        // Get part file
        $partfile =
            dirname(__FILE__) . "/../views/layouts/{$layout}_layout.php";
        if (!file_exists($partfile)) {
            // Fall back to default layout
            $partfile =
                dirname(__FILE__) . "/../views/layouts/default_layout.php";
        }
    ...
    public function render_page()
    {
        ...
        if (
            in_array($this->action, $this->pages_cached) &&
    ...
    public function render_component($params)
    {
        ...
        if (!isset($params['controller'])) {
            ...
            require_once dirname(__FILE__) .
                "/../controllers/{$controller_name}_controller.php";
        ...
        // Create class name
        $classname =
            joined_lower_to_camel_case($controller_name) . "Controller";
