...
class MysqliAdapter implements DbAdapter
{
    ...
    public function exec()
    {
        ...
        try {
            ...
            $msg = $mse->getMessage();
            if (str_starts_with($msg, 'Duplicate entry')) {
                $exception_cls = DuplicateEntryException::class;
            }
            throw new $exception_cls(
                Config::get('DB_DEBUG') ? "Error ({$this->query}): {$msg}" : $msg,
                $mse->getCode(),
                $mse
            );
        }
        ...
        if (!$this->result) {
            throw new Exception(
                Config::get('DB_DEBUG') ? "Error ({$this->query}): {$this->link->error}" : $this->link->error
            );
        }
