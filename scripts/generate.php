...
function create_view($controller, $action)
{
	...
	file_put_contents(dirname(__FILE__) . "/../views/$controller/$action.php", '<!-- TODO: add your code here -->');
}
...
EOT;
		...
		file_put_contents(dirname(__FILE__) . "/../controllers/{$controller}_controller.php", $controller_code);
	} elseif ($argv[1] == 'model') {
...
EOT;
		...
		file_put_contents(dirname(__FILE__) . "/../models/$model.php", $model_code);
